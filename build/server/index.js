...
wss.on('connection', function (ws) {
  ...
  if (docName in documents) {
    ...
    doc = gulf.Document.create(new gulf.MemoryAdapter(), textOT, docContents, function (er, doc) {
      if (er) {
        log.error('Error creating doc:', er);
      }

      // Attach some listeners for logging
      doc.on('init', function () {
        log.debug('New doc initialized', docName);
      });
      doc.on('edit', function () {
        log.debug('Edit received for doc', docName);
      });
  ...
  link.on('data', function (data) {
    ...
    ws.send(data, function (err) {
      if (err) {
        log.error('Error sending data to WebSocket', data, err);
      }
    });
  });
